...
int main()
{
    ...
    cout<<"DFA最小化划分的子集如下："<<endl;
    for(i=0;i<flag;i++)
    {
        if(part[i]!="")cout<<part[i]<<endl;
    }
    cout<<"用状态P,Q···等代替子集："<<endl;
    for(i=0;i<flag;i++)
    {
        if(part[i]!=""){
            if(i == flag - 1)cout<<"{"<<part[i]<<"}";
            else cout<<"{"<<part[i]<<"},";
        }
    }
    ...
    cout<<"最小化后终态："<<newFinal<<endl;
    for(i = 0;i < Condition.size();i++){
        if(i != Condition.length()-1)fout<<Condition[i]<<" ";
        else fout<<Condition[i]<<" `"<<endl;
    }
    ...
    for(i=0;i < resCnt;i++){
        for(j = 0;j < Condition.length() + 1;j++){
            if(j != Condition.length())fout<<output[i][j]<<" ";
            else fout<<output[i][j]<<" `"<<endl;
        }
    }
    fout<<"#"<<" `"<<endl;
    fout<<newFinal<<endl;
